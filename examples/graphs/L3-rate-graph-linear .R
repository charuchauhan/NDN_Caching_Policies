...
#########################
data = read.table("L3_rate_linear_trace.txt", header=T)
...
data.leaves = subset(data.combined, Node %in% c("0","1", "2", "3", "4","5","6"))
...
png("src/ndnSIM/docs/source/_static/L3-rates-linear.png", width=500, height=250)
print(g.root)
retval <- dev.off()
